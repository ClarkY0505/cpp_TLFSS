...
/**
 * @brief CliRegistry::register_command() 的执行结果。
 */
enum class CliRegisterStatus : std::uint8_t {
  ...
  /**
   * @brief 命令名为空，或者包含空白、回车、换行等非法字符。
   */
  INVALID_NAME,

  /**
   * @brief 调用者传入了空的 std::function。
   */
  INVALID_HANDLER,

  /**
   * @brief 注册表中已经存在完全相同的命令名。
   */
  DUPLICATE_COMMAND
};
...
enum class CliDispatchStatus : std::uint8_t {
  /**
   * @brief 找到命令，并且 Handler 正常返回。
   */
  SUCCESS,

  /**
   * @brief 输入为空，或者输入只包含空白字符。
   */
  EMPTY_INPUT,

  /**
   * @brief 第一个 token 没有对应的已注册命令。
   */
  UNKNOWN_COMMAND,

  /**
   * @brief token 数超过 CLI_MAX_TOKENS。
   */
  TOO_MANY_TOKENS,

  /**
   * @brief 输入行长度超过 CLI_MAX_LINE_SIZE。
   */
  LINE_TOO_LONG,
...
/** @brief 命令名之后的参数列表；分派器保留原始 token 顺序。 */
using CliArguments = std::vector<std::string>;

/**
 * @brief 单个 TCP 客户端连接独立拥有的状态。
 *
...
struct CliSessionContext final {
  ...
  /**
   * @brief 判断当前 Session 是否选择了具体模块。
   *
   * mid 是模块过滤的权威状态；名称仅用于显示。
   * @return 当前会话选中具体模块时返回 true。
   */
...
/** @brief 可读取和修改当前客户端 Session 状态的处理函数。 */
